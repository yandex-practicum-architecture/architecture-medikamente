...
•  Текущая система (As-Is): Компания «Медикаменте» в настоящее время использует сильно ручные процессы для работы с данными:
...
•  Цели миграции (To-Be):
  ...
  •  Новые сервисы: Интеграция с лабораторией анализов через API, разработка BI/ML/AI-сервисов на базе озера данных, создание клиентского и рецепционного порталов, платёжного шлюза, CRM, мобильного приложения.
...
* Отсутствие DLP, аудита и мониторинга создает риски утечек данных и компрометации системы.
...
| **API Gateway:** Перегрузка и риск деградации                                                         | 1.

Масштабирование API Gateway (autoscaling, rate limiting, circuit breakers). 2. Нагрузочное тестирование. 3. Payload validation на шлюзе.                                                          | Средний  |
...
*   **Ключевые проблемы:** Обобщение проблем из Шага 4.
*   **Ключевые решения:** Обобщение рекомендаций из Шага 5.

**8. Контроль внедрения (Метрики и сигналы тревоги)**
